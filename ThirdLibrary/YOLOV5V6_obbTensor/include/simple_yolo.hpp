...
namespace SimpleYolo{
    ...
    struct Box{
        ...
        Box(float left, float top, float right, float bottom, float confidence, int class_label, Type type)
        :left(left), top(top), right(right), bottom(bottom), confidence(confidence), class_label(class_label), type(type){}

        Box(float cx, float cy, float width, float height, float confidence, int class_label, float theta, Type type)
        :cx(cx), cy(cy), width(width), height(height), confidence(confidence), class_label(class_label), theta(theta), type(type){}
    };
